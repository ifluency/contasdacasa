...
export async function POST(req: Request) {
  try {
    ...
    const [txRules, incRules] = await Promise.all([loadRules("TRANSACTION"), loadRules("INCOME")]);
    ...
    for (const p of parsed) {
      ...
      if (isAccount && isPositive) {
        const act = applyRulesFromList(incRules, originalDesc);
      ...
      const act = applyRulesFromList(txRules, originalDesc);
