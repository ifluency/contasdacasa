import type { Prisma, Person, PaymentType } from "@prisma/client";
...
function sumAbsCents(rows: TxWithCategory[]): number {
  let s = 0;
  for (const t of rows) s += Math.abs(t.amountCents);
  return s;
}

function safeName(t: TxWithCategory): string {
  return t.normalized?.trim() ? t.normalized : t.description;
}
...
function TxTable({ title, rows, showParcelInfo }: { title: string; rows: TxWithCategory[]; showParcelInfo?: boolean }) {
  return (
...
function filterPersonTx(allTx: TxWithCategory[], person: DashPerson) {
  return allTx.filter((t) => t.person === person);
}

function filterByType(allTx: TxWithCategory[], type: PaymentType) {
...
export default async function Page({ searchParams }: { searchParams?: { mes?: string } }) {
  ...
  type PersonSums = { salario: number; vale: number; outros: number; restante: number; total: number };
  const incomeByPerson: Record<DashPerson, PersonSums> = {
  ...
  function computePersonSummary(person: DashPerson) {
